          ...
          <>
            <Reveal variant="fade" delay={0.1}>
              <div className="flex items-center justify-between mb-6">
                ...
                {data?.model && (
                  <span className="px-3 py-1 bg-cyan-600/20 text-cyan-400 text-xs font-semibold rounded-full">
                    {data.model}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-4">
              {data?.movers?.map((stock: any, idx: number) => (
                <Reveal key={`strongest-${stock.symbol || 'stock'}-${idx}`} variant="rise" delay={idx * 0.05}>
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              ...
                              {stock.relative_strength_score && (
                                <span className="px-2 py-1 bg-purple-600/20 text-purple-400 text-xs font-semibold rounded">
                                  Score: {stock.relative_strength_score.toFixed(0)}
                                </span>
                              )}
                            </div>
                            {stock.rationale && (
